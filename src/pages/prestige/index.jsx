...
function Prestige() {
    ...
    const prevPrestige = useMemo(() => {
        ...
        return (
            <div className="related-quests" key={"prev-prestige"}>
                <h3>
                    <Icon path={mdiArrowLeftBold} size={1} className="icon-with-text" /> {t("Previous Prestige")}
                </h3>
                <div key={`req-task-${prev.id}`}>
                    <Link to={`/prestige/${prev.prestigeLevel}`}>
                        <img src={prev.iconLink} alt="" className="prestige-icon" />
                        {prev.name}
                    </Link>
                </div>
            </div>
    ...
    const nextPrestige = useMemo(() => {
        ...
        if (
            ...
            currentPrestige.prestigeLevel >= maxPrestige.prestigeLevel
        ) {
            return;
        }
        ...
        return (
            <div className="related-quests" key={"next-prestige"}>
                <h3>
                    <Icon path={mdiArrowRightBold} size={1} className="icon-with-text" /> {t("Next Prestige")}
                </h3>
                <div key={`req-task-${next.id}`}>
                    <Link to={`/prestige/${next.prestigeLevel}`}>
                        <img src={next.iconLink} alt="" className="prestige-icon" />
                        {next.name}
                    </Link>
                </div>
            </div>
    ...
    const gameModeWarning = useMemo(() => {
        ...
        return (
            <p className="game-mode-warning">
                {t(`Prestige conditions may not display properly while viewing in {{gameModeName}} mode.`, {
                    gameModeName: t(`game_mode_${gameMode}`),
                })}
            </p>
        );
    }, [gameMode, t]);
    ...
    const stashTransferSettings = useMemo(() => {
        ...
        return (
            <>
                <h3>
                    {t("Item Transfer")}: {`${stashTransfer.gridWidth} x ${stashTransfer.gridHeight}`}
                </h3>
                <div key="item-transfer-settings">
                    ...
                    <ul>
                        {stashTransfer.itemFilters.allowedCategories
                            .map((allowedCat) => {
                                ...
                                if (
                                    stashTransfer.itemFilters.excludedCategories.some(
                                        (excludedCat) => category.id === excludedCat.id,
                                    )
                                ) {
                                    return false;
                                }
                    ...
                    <ul className="quest-item-list">
                        {stashTransfer.itemFilters.allowedItems
                            .map((allowedItem) => {
                                ...
                                if (
                                    stashTransfer.itemFilters.excludedItems.some(
                                ...
                                if (
                                    stashTransfer.itemFilters.excludedCategories.some((excludedCat) =>
                                        item.categories.some((cat) => cat.id === excludedCat.id),
                                    )
                                ) {
                                    return false;
                                }
                            ...
                            .map((item, index) => {
                                return (
                                    <li key={`allowed-item-${item.id}-${index}`}>
                                        <ItemImage
                    ...
                    {!!stashTransfer.itemFilters.excludedItems.length && (
                        <>
                            ...
                            <ul className="quest-item-list">
                                {stashTransfer.itemFilters.excludedItems
                                    .map((excludedItem) => {
                                    ...
                                    .map((item, index) => {
                                        return (
                                            <li key={`excluded-item-${item.id}-${index}`}>
                                                <ItemImage
    ...
    return [
        <SEO
            ...
            description={t(
                ...
                "This page includes information on the conditions and rewards for obtaining {{prestigeName}}.",
                { prestigeName: currentPrestige.name },
            )}
        ...
        <div className="display-wrapper" key={"display-wrapper"}>
            <div className={"entity-page-wrapper"} key={"quest-page-display-wrapper"}>
                    ...
                    <div className="entity-top-content">
                        ...
                        <div className="title-bar">
                            ...
                            <span className="wiki-link-wrapper">
                                <a
                                    href={"https://escapefromtarkov.fandom.com/wiki/Prestige"}
                                    target="_blank"
                        ...
                        <div className="main-content">{gameModeWarning}</div>
                ...
                <div className="information-section has-table">
                    <h2>
                        <Icon path={mdiFormatListCheckbox} size={1.5} className="icon-with-text" /> {t("Conditions")}
                    </h2>
                ...
                {Object.keys(currentPrestige.rewards).some((r) => currentPrestige.rewards[r]?.length) && (
                    <div key="task-start-rewards" className="information-section has-table">
                        <h2>
                            <Icon path={mdiGift} size={1.5} className="icon-with-text" /> {t("Rewards")}
                        </h2>
                        <div key="task-start-rewards-content" className="information-content">
                            {TaskRewards({
                                rewards: currentPrestige.rewards,
                                t,
                                items,
                                settings,
                                traders,
                ...
                <div className="information-section has-table">
                    <h2>
                        <Icon path={mdiRedo} size={1.5} className="icon-with-text" /> {t("Transfer Settings")}
                    </h2>
                    <div className="information-content">
                        <h3>{t("Skill Transfer")}</h3>
                        <div key="transfer-settings-skills">
                            <ul>
                                {currentPrestige.transferSettings
                                    .filter((t) => !!t.skillType)
                                    .map((skillTransfer) => {
                                        return (
                                            <li key={`skill-transfer-${skillTransfer.skillType}`}>
                                                {skillTransfer.name}: {skillTransfer.transferRate * 100}%
                                            </li>
                                        );
                                    })}
                            </ul>

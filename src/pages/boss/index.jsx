...
function BossPage(params) {
            ...
            },
            {
                ...
                Cell: (props) => {
                    if (bosses.some((boss) => boss.normalizedName === props.row.original.normalizedName)) {
                        return (
                            <CenterCell>
                                <Link to={`/boss/${props.row.original.normalizedName}`}>{props.value}</Link>
                            </CenterCell>
                        );
                    }
    ...
    const bossProperties = {
        usedOnMaps: {
            value: bossData.maps.reduce((bossMaps, current) => {
            ...
            label: (
                <span>
                    {t("Map")} <Icon path={mdiMapLegend} size={1} className="icon-with-text" />
                </span>
            ),
            order: 2,
        },
    ...
    if (spawnStatsMsg.length > 0) {
        bossProperties["spawnChance"] = {
            value: spawnStatsMsg.reduce((prev, curr, currentIndex) => [
                prev,
            ...
            label: (
                <span>
                    {t("Spawn chance")} <Icon path={mdiDice5} size={1} className="icon-with-text" />
                </span>
            ),
    ...
    if (bossData.health) {
        ...
        bossProperties["bodyPartsHealth"] = {
            ...
            label: (
                <span>
                    {t("Health")} ({totalHealth}) <Icon path={mdiHeart} size={1} className="icon-with-text" />
                </span>
            ),
    ...
    if (bossData.behavior) {
        bossProperties["behavior"] = {
            ...
            label: (
                <span>
                    {t("Behavior")} <Icon path={mdiBrain} size={1} className="icon-with-text" />
                </span>
            ),
        ...
        report = (
            <div>
                <h2>
                    ...
                    <Icon path={mdiInvoiceTextClockOutline} size={1.5} className="icon-with-text" />
                </h2>
                <ul>
                    {bossData.reports.map((report, index) => {
                        ...
                        return (
                            <li key={`report-${index}`}>
                                {reportedMap.name}:{" "}
                                {t("{{val, relativetime}}", { val: relativeTime[0], range: relativeTime[1] })}
                            </li>
                        );
                    })}
                </ul>
                    ...
                    <div className="entity-top-content">
                        ...
                        <div className="main-content">
                            ...
                            {i18n.exists(`${bossData.normalizedName}-description`, { ns: "bosses" }) &&
                                t(`${bossData.normalizedName}-description`, { ns: "bosses" }).length > 0 && (
                                    <div>
                                        <h3>{t("Behavior")}</h3>
                                        <p className="entity-details">
                                            {/* prettier-ignore */}

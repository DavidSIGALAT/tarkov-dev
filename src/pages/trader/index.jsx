...
import { mdiArmFlex, mdiCashSync, mdiChartLine } from "@mdi/js";

import useStateWithLocalStorage from "../../hooks/useStateWithLocalStorage.jsx";

import SEO from "../../components/SEO.jsx";
import { Filter, InputFilter, ButtonGroupFilter, ButtonGroupFilterButton } from "../../components/filter/index.jsx";
...
function Trader() {
    ...
    const setPathFilters = useCallback(
        (filtervalues) => {
            const params = {
                search: nameFilter,
                tab: selectedTable,
    ...
    const levelProperties = useMemo(() => {
        ...
        if (levelInfo.requiredPlayerLevel > 1) {
            props.requiredPlayerLevel = {
                ...
                label: (
                    <span>
                        {t("Player level")} <Icon path={mdiArmFlex} size={1} className="icon-with-text" />
                    </span>
                ),
            };
        }
        props.requiredReputation = {
            ...
            label: (
                <span>
                    {t("Reputation")} <Icon path={mdiChartLine} size={1} className="icon-with-text" />
                </span>
            ),
        };
        if (levelInfo.requiredCommerce > 0) {
            props.requiredCommerce = {
                ...
                label: (
                    <span>
                        {t("Commerce")} <Icon path={mdiCashSync} size={1} className="icon-with-text" />
                    </span>
                ),
            };
        }
            ...
            <div className={"entity-page-wrapper"} key={"trader-page-display-wrapper"}>
                <div className="entity-information-wrapper">
                    <div className="entity-top-content">
                        ...
                        <div className="title-bar">
                            <span className="type">{t("Trader")}</span>
                            <h1>{trader.name}</h1>
                        ...
                        <div className="main-content">
                            <p className="entity-details">{trader.description}</p>
                        </div>
                ...
                <div className="page-headline-wrapper">
                    ...
                    <Filter center>
                        {buyBackTableEnabled ? (
                            <ButtonGroupFilter>
                                <ButtonGroupFilterButton
                                    tooltipContent={<>{t("Items with the best cash back prices for leveling")}</>}
                        ...
                        <ButtonGroupFilter>
                            <ButtonGroupFilterButton
                                tooltipContent={<>{t("Tasks given by {{traderName}}", { traderName: trader.name })}</>}

...
const markerTypes = ["Plus", "Circle", "TriangleUp", "TriangleDown", "Diamond"];
...
function Ammo() {
    ...
    const { ammoData, legendData } = useMemo(() => {
        const typeCache = {};
        const legend = [];
        const calibers = items
            .filter((item) => {
                return (
                    item.categoryIds.includes(ammoCategoryId) &&
                    !!item.properties.caliber &&
                    !skipCalibers.includes(item.properties.caliber)
                );
            })
            .reduce((all, current) => {
            ...
            .map((item) => {
                const returnData = {
                    ...
                    displayDamage: useAllProjectileDamage
                        ? item.properties.projectileCount * item.properties.damage
        ...
        <div className="display-wrapper" key="ammo-wrapper">
            ...
            <Filter>
                ...
                <ToggleFilter
                    ...
                    label={t("Trader Ammo")}
                    onChange={(e) => setShowOnlyTraderAmmo(!showOnlyTraderAmmo)}
                    tooltipContent={<>{t("Only show ammo available from traders on your settings")}</>}
                />
                <ToggleFilter

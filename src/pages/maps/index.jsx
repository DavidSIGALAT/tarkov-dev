    ...
    return [
        <SEO
            ...
            description={t(
                ...
                "Get the latest information on all maps in Escape from Tarkov, including extract points and loot locations. Find out where to find the best gear and resources in the game",
            )}
            key="seo-wrapper"
        />,
        <div className={"page-wrapper"} key="map-page-wrapper">
            ...
            {uniqueMaps.map((mapsGroup) => {
                return (
                    <div
                        key={mapsGroup.normalizedName}
                        id={mapsGroup.normalizedName}
                        className="information-section map-block"
                    >
                        <h2>
                            <span className="icon">
                                <Icon path={mapIcons[mapsGroup.normalizedName]} size={1} className="icon-with-text" />
                            </span>
                            {
                                // t('Streets of Tarkov')
                                // t('Ground Zero')
                        ...
                        <div className="page-wrapper map-page-wrapper">{mapsGroup.description}</div>
                                ...
                                .map((map) => {
                                    ...
                                    if (map.projection === "interactive") {
                                        let path =
                                            map.svgPath ||
                                            map.tilePath ||
                                            `https://assets.tarkov.dev/maps/${map.normalizedName}/{z}/{x}/{y}.png`;

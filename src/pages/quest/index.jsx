...
function Quest() {
    ...
    const endgameGoals = useMemo(() => {
        ...
        if (currentQuest?.kappaRequired) {
            goals.push(
                <Tooltip key={`${currentQuest.id}-kappa`} title={t("Required for Kappa")} placement={"top"} arrow>
                    <Link to={"/task/collector"}>
                        <Icon path={mdiBriefcase} size={0.75} className="icon-with-text" />
                    </Link>
                </Tooltip>,
            );
        }
            ...
            goals.push(
                ...
                >
                    <Link to={"/task/knock-knock"}>
                        <Icon path={mdiLighthouse} size={0.75} className="icon-with-text" />
                    </Link>
                </Tooltip>,
            );
        }
    ...
    const taskStatusIcon = useMemo(() => {
        ...
        return (
            <Tooltip key={`${currentQuest.id}-status-icon`} title={iconTitle} placement={"top"} arrow>
                <Icon path={iconPath} size={0.75} className="icon-with-text" />
            </Tooltip>
        );
    ...
    const getTaskStatusIcon = useCallback(
        (status, options = {}) => {
            ...
            return (
                <Tooltip title={options.tooltip ?? t(status)} key={options.key ?? `task-status-${status}`} arrow>
    ...
    const previousTasks = useMemo(() => {
        ...
        return (
            <div className="related-quests" key={"task-status-req"}>
                <h3>
                    <Icon path={mdiCheckBold} size={1} className="icon-with-text" /> {t("Prerequisite Tasks")}
                </h3>
                {currentQuest.taskRequirements.map((taskReq) => {
                    ...
                    return (
                        <div key={`req-task-${task.id}`}>
                            <Icon path={taskIcon} size={1} className="icon-with-text" />
    ...
    const nextTasks = useMemo(() => {
        ...
        const nextQuests = quests.filter((quest) =>
            quest.taskRequirements.some(
                (req) =>
                    req.task.id === currentQuest.id &&
                    (!req.status.includes("active") ||
                        (req.status.length === 2 && req.status.includes("complete") && req.status.includes("active"))),
            ),
        );
        ...
        return (
            <div className="related-quests" key="next-tasks">
                <h3>
                    <Icon path={mdiChevronRight} size={1} className="icon-with-text" /> {t("Leads to")}
                </h3>
                {nextQuests.map((task) => {
                    let status = task.taskRequirements.find((req) => req.task.id === currentQuest.id).status;
                    let taskStatuses = [];
                    if (status.length > 1 || status[0] !== "complete") {
                        taskStatuses = status.map((s) =>
                            getTaskStatusIcon(s, { tooltip: t("This task {{taskStatus}}", { taskStatus: t(s) }) }),
                        );
                    }
                    let taskIcon = mdiClipboardList;
    ...
    const alternateTasks = useMemo(() => {
        ...
        const altQuests = quests.filter((q) =>
            currentQuest.failConditions.some(
                (objective) =>
        ...
        return (
            <div className="related-quests" key="alternate-quests">
                <h3>
                    <Icon path={mdiSourceBranch} size={1} className="icon-with-text" /> {t("Other Options")}
                </h3>
                {altQuests.map((task) => {
                    let taskIcon = mdiClipboardList;
                    if (settings.failedQuests.includes(task.id)) {
        ...
        <div className="display-wrapper" key={"display-wrapper"}>
            <div className={"entity-page-wrapper"} key={"quest-page-display-wrapper"}>
                ...
                <div className="entity-information-wrapper">
                    <div className="entity-top-content">
                        ...
                        <div className="title-bar">
                            ...
                            <h1>
                                {currentQuest.name} {taskStatusIcon}
                                {endgameGoals}
                            </h1>
                    ...
                    <div className="entity-icon-cont">
                        <div
                            className="entity-icon-and-link-wrapper"
                            onClick={() => openImageViewer(0)}
                ...
                <div className="information-section has-table">
                    <h2>
                        <Icon path={mdiFormatListCheckbox} size={1.5} className="icon-with-text" /> {t("Objectives")}
                    </h2>
                ...
                {currentQuest.failConditions?.length > 0 && (
                    <div className="information-section has-table">
                        <h2>
                            <Icon path={mdiCloseThick} size={1.5} className="icon-with-text" /> {t("Fail On")}
                        </h2>
                ...
                {currentQuest.neededKeys?.length > 0 && (
                    <div key="task-keys" className="information-section has-table">
                        <h2>
                            <Icon path={mdiKeyVariant} size={1.5} className="icon-with-text" /> {t("Needed Keys")}
                        </h2>
                        <ul>
                ...
                {Object.keys(currentQuest.startRewards).some((r) => currentQuest.startRewards[r]?.length) && (
                    <div key="task-start-rewards" className="information-section has-table">
                        <h2>
                            <Icon path={mdiPlay} size={1.5} className="icon-with-text" /> {t("Starting Equipment")}
                        </h2>
                        <div key="task-start-rewards-content" className="information-content">
                            {TaskRewards({
                                rewards: currentQuest.startRewards,
                                t,
                                items,
                                settings,
                                traders,
                ...
                {hasFailInfo && (
                    <div>
                        <div key="task-failure-rewards" className="information-section has-table">
                            <h2>
                                <Icon path={mdiCloseThick} size={1.5} className="icon-with-text" />{" "}
                                {t("Failure Penalties")}
                            </h2>
                            <div key="task-failure-rewards-content-restartable" className="information-content">
                                {currentQuest.restartable ? t("Can be restarted") : t("Cannot be restarted")}
                            </div>
                            <div key="task-failure-rewards-content" className="information-content">
                                {TaskRewards({
                                    rewards: currentQuest.failureOutcome,
                                    t,
                                    items,
                                    settings,
                                    traders,

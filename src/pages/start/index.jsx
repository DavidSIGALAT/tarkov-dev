...
const ServerStatus = lazy(() => import("../../components/server-status/index.jsx"));
const SmallItemTable = lazy(() => import("../../components/small-item-table/index.jsx"));
const ItemSearch = lazy(() => import("../../components/item-search/index.jsx"));
const BossList = lazy(() => import("../../components/boss-list/index.jsx"));
...
function Start() {
    ...
    return [
        <SEO
            ...
            key="seo-wrapper"
        />,
        <div className="display-wrapper page-wrapper start-wrapper" key={"display-wrapper-start-page"}>
                ...
                <Suspense fallback={<LoadingSmall />}>
                    ...
                    {!loadMoreState && [
                        <div className="load-more-wrapper" key="start-load-more-button">
                            <button id="load-more-button" className="load-more-button" onClick={loadMore}>
                                {t("Load More")}
                            </button>
                        </div>,
                    ]}
            ...
            <div className="start-section-wrapper" key={"sidebar-div"}>
                ...
                <ul className="tools-list" key="tools-list">
                    ...
                    <li key="start-link-barters">
                        <Link to="/barters/">
                            <Icon path={mdiCached} size={1} className="icon-with-text" />
                    ...
                    <li key="start-link-tarkov-monitor">
                        <Link to="/tarkov-monitor">
                            <Icon path={mdiMonitorCellphone} size={1} className="icon-with-text" />
                            {t("TarkovMonitor")}
                        </Link>
                    </li>
                    <li key="start-link-stash-page">
                        <Link to="/stash-discord-bot">
                            <Icon path={mdiRobot} size={1} className="icon-with-text" />
                            {t("Stash Discord Bot")}
                        </Link>
                    </li>
                </ul>
                        ...
                        return (
                            <li key={`start-link-to-${categoryPage.key}`}>
                                <Link to={`/items/${categoryPage.key}`}>
                                    <Icon path={ItemIconList(categoryPage.icon)} size={1} className="icon-with-text" />
                ...
                <h3>
                    <Link to={"/bosses"} key={"bosses-page"}>
                        <Icon path={mdiSkull} size={1} className="icon-with-text" />
                        {t("Bosses")}
                    </Link>
                </h3>

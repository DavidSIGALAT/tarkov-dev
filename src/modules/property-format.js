...
const itemCategoryLinkFormat = (inputCategory) => {
    ...
    return (
        <Link to={`/items/handbook/${inputCategory.normalizedName}`} key={inputCategory.normalizedName}>
            {categoryImage}
            {inputCategory.name}
        </Link>
    );
...
const formatter = (key, value, id) => {
    ...
    if (key === "bodyPartsHealth" && value.length > 0) {
        value = value
            ?.map((b) => {
                return (
                    <span key={`bodypart=${b.id}`}>
                        <span>{`${b.bodyPart}: `}</span>
                        <Link to={`/ammo/?mind=${b.max}`}>{b.max}</Link>
                    </span>
                );
            })

...
export function CustomizationReward(reward, items) {
    ...
    return (
        <div key={reward.id} className="customization-image">
...
export function TaskObjective({ objective, items, bosses, quests, traders, maps, handbook, settings, stations, t }) {
    ...
    if (objective.type === "buildWeapon") {
        ...
        taskDetails = (
            <>
                <>
                    <ItemImage item={baseItem} imageField="baseImageLink" linkToItem={true} />
                </>
                {attributes.length > 0 && (
                    <>
                        <h4>{t("Attributes")}</h4>
    ...
    if (objective.type === "extract") {
        ...
        if (objective.exitName) {
            extract = <div>{t("using extract: {{extractName}}", { extractName: objective.exitName })}</div>;
        }
    ...
    if (objective.type === "plantItem") {
        ...
        if (objective.items.length > 1) {
            ...
            taskDetails = (
                <>
                    <ItemImage item={item} imageField="baseImageLink" linkToItem={true} count={plantCount} />
                </>
            );
        }
    }
    if (objective.type === "shoot") {
        ...
        const targets = objective.targetNames
            .map((t) => {
                ...
                return (
                    <Link key={`boss-${t}`} to={`/boss/${boss.normalizedName}`}>
                        {t}
                    </Link>
                );
            })
    ...
    if (objective.type === "playerLevel") {
        taskDetails = <div>{t("Reach level {{playerLevel}}", { playerLevel: objective.playerLevel })}</div>;
    }
    ...
    const objectiveDescription = (
        <h3>
            <Icon path={objectiveIconPath} size={1} className="icon-with-text" />
            {objectiveTitle}
        </h3>
    );
    if (objective.zones?.length > 0) {
        mapQuery = objective.zones
            .reduce((ids, z) => {

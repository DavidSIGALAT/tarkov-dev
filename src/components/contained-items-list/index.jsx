...
const ContainedItemsList = ({ item, showRestrictedType }) => {
    ...
    }, [items, handbook, handbookStatus, containers, item, showRestrictedType]);
    ...
    return (
        <div>
            ...
            {holdItems.map((linked, index) => {
                ...
                return (
                    <span className="contained-item-link-wrapper" key={`item-link-${item.id}-${linked.id}`}>
                        <Link className="contained-item-link" to={`/item${isCategory}/${linked.normalizedName}`}>
                            {linked.name}
                        </Link>
                        <span>{holdItems.length > index + 1 ? "," : ""}</span>
                    </span>
                );
            })}
        </div>
